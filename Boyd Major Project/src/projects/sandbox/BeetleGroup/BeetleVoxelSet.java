...
public class BeetleVoxelSet extends VoxelGrid{


	public BeetleVoxelSet(int _w, int _h, int _d, Vec3D _s) {
		super(_w, _h, _d, _s);
		...
		for (int i = 0; i<w; i++) {
			for (int j = 0; j<h; j++) {
				for (int k = 0; k<d; k++) {
					...
					vals[index] = new Cell(1, edge, i, j, k);
				}
			}
		}
	}
