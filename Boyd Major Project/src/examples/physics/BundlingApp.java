...
public class BundlingApp extends PApplet{
		// ---------------------------------------------------------------------------EXTERNAL LIBS
		PeasyCam cam;
		ControlP5 controlP5;
		...
		public void setup(){
			...
			environment.addAgents(IO.importLinkedPlanes(this,"links9.txt"));
			environment.update(true);
			
			//add some agents
			/*
			for(int i=0;i<50;i++){
				DemoPhysicsAgent a = new DemoPhysicsAgent(new Vec3D(random(100),random(100),0),false);
		...
		public void draw(){
			...
			environment.update(true);
			environment.bundle(3,0.1f); //do the bundling
